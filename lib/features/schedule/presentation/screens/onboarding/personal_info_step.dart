...
class PersonalInfoStep extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final fullNameController = useTextEditingController(
      text: state.formData.fullName ?? '',
    );
    final phoneController = useTextEditingController(
      text: state.formData.phoneNumber ?? '',
    );
    final emailController = useTextEditingController(
      text: state.formData.email ?? '',
    );
    final streetController = useTextEditingController(
      text: state.formData.streetAddress ?? '',
    );
    final zipController = useTextEditingController(
      text: state.formData.zipCode ?? '',
    );

    final selectedState = useState<String?>(state.formData.state);
    final selectedCity = useState<String?>(state.formData.city);
